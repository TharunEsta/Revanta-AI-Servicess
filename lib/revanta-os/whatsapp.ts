...
function buildHumanHandoffMessage(language: "ENGLISH" | "TELUGU", bookingUrl?: string | null) {
  if (language === "TELUGU") {
    return bookingUrl
      ? `α░ºα░¿α▒ìα░»α░╡α░╛α░ªα░╛α░▓α▒ü. α░«α▒Ç α░àα░¡α▒ìα░»α░░α▒ìα░Ñα░¿α░¿α▒ü α░«α░╛ α░ƒα▒Çα░«α▒ìΓÇîα░òα▒ü α░¬α░éα░¬α░┐α░éα░Üα░╛α░«α▒ü.\n\nα░«α▒Çα░òα▒ü α░╕α▒îα░òα░░α▒ìα░»α░éα░ùα░╛ α░ëα░éα░ƒα▒ç α░ê α░▓α░┐α░éα░òα▒ìΓÇîα░▓α▒ï α░íα░┐α░╕α▒ìα░òα░╡α░░α▒Ç α░òα░╛α░▓α▒ì α░¼α▒üα░òα▒ì α░Üα▒çα░╕α▒üα░òα▒ïα░╡α░Üα▒ìα░Üα▒ü:\n${bookingUrl}`
      : "α░ºα░¿α▒ìα░»α░╡α░╛α░ªα░╛α░▓α▒ü. α░«α▒Ç α░àα░¡α▒ìα░»α░░α▒ìα░Ñα░¿α░¿α▒ü α░«α░╛ α░ƒα▒Çα░«α▒ìΓÇîα░òα▒ü α░¬α░éα░¬α░┐α░éα░Üα░╛α░«α▒ü. α░«α░╛ α░ƒα▒Çα░«α▒ì α░ñα▒ìα░╡α░░α░▓α▒ï α░«α▒Çα░ñα▒ï α░╕α░éα░¬α▒ìα░░α░ªα░┐α░╕α▒ìα░ñα▒üα░éα░ªα░┐.";
  }
...
function buildMeetingLinkAcknowledgementMessage(params: {
  ...
  if (params.language === "TELUGU") {
    return params.bookingUrl
      ? `α░«α▒Çα░░α▒ü α░«α▒Çα░ƒα░┐α░éα░ùα▒ì α░▓α░┐α░éα░òα▒ì α░╖α▒çα░░α▒ì α░Üα▒çα░╕α░┐α░¿α░éα░ªα▒üα░òα▒ü α░ºα░¿α▒ìα░»α░╡α░╛α░ªα░╛α░▓α▒ü.\n${params.sharedLink}\n\nα░«α░╛ α░ƒα▒Çα░«α▒ì α░ªα▒Çα░¿α░┐α░¿α░┐ α░╕α░«α▒Çα░òα▒ìα░╖α░┐α░╕α▒ìα░ñα▒üα░éα░ªα░┐. α░«α▒Çα░░α▒ü α░çα░╖α▒ìα░ƒα░¬α░íα░┐α░ñα▒ç α░«α░╛ α░íα░┐α░╕α▒ìα░òα░╡α░░α▒Ç α░òα░╛α░▓α▒ìΓÇîα░¿α▒ü α░òα▒éα░íα░╛ α░çα░òα▒ìα░òα░í α░¼α▒üα░òα▒ì α░Üα▒çα░╕α▒üα░òα▒ïα░╡α░Üα▒ìα░Üα▒ü:\n${params.bookingUrl}`
      : `α░«α▒Çα░░α▒ü α░«α▒Çα░ƒα░┐α░éα░ùα▒ì α░▓α░┐α░éα░òα▒ì α░╖α▒çα░░α▒ì α░Üα▒çα░╕α░┐α░¿α░éα░ªα▒üα░òα▒ü α░ºα░¿α▒ìα░»α░╡α░╛α░ªα░╛α░▓α▒ü.\n${params.sharedLink}\n\nα░«α░╛ α░ƒα▒Çα░«α▒ì α░ªα▒Çα░¿α░┐α░¿α░┐ α░╕α░«α▒Çα░òα▒ìα░╖α░┐α░éα░Üα░┐ α░ñα▒ìα░╡α░░α░▓α▒ï α░«α▒Çα░ñα▒ï α░╕α░éα░¬α▒ìα░░α░ªα░┐α░╕α▒ìα░ñα▒üα░éα░ªα░┐.`;
  }
...
type WhatsAppIntegrationLookup = {
  integration: Awaited<ReturnType<typeof prisma.whatsAppIntegration.findFirst>>;
  strategy: "phoneNumberId" | "businessAccountId" | "singleIntegrationFallback" | "none";
};

async function findWhatsAppIntegrationByIdentifiers(params: {
  phoneNumberId?: string | null;
  businessAccountId?: string | null;
}): Promise<WhatsAppIntegrationLookup> {
  const phoneNumberId = params.phoneNumberId?.trim() || null;
  const businessAccountId = params.businessAccountId?.trim() || null;
  ...
  if (phoneNumberId) {
    ...
    if (integration) {
      return { integration, strategy: "phoneNumberId" };
    }
  }
  ...
  if (businessAccountId) {
    ...
    if (integration) {
      return { integration, strategy: "businessAccountId" };
    }
  }
  ...
  if (integrations.length === 1) {
    return { integration: integrations[0], strategy: "singleIntegrationFallback" };
  }

  return { integration: null, strategy: "none" };
}
...
async function findOrCreateContact(params: {
  ...
  const existing = await prisma.contact.findFirst({
    where: {
      organizationId: params.organizationId,
      OR: [{ phone: normalizedPhone }, { phone: params.phone }, { leadId: params.leadId }]
    }
  });
...
function toPersonalReply(text: string) {
  ...
  if (/\bbook\b|\bschedule\b|\bdiscovery\b|\bcall\b|\bdemo\b|\bpricing\b/i.test(t)) {
    return `Cheppandi bro, okati fix cheddam ≡ƒÿä\n\n${t}`;
  }
  if (/\bWould you like\b/i.test(t)) {
    return `Cheppandi bro ≡ƒÿä ${t}`;
  }
  if (/\bWelcome to\b/i.test(t)) {
    return t.replace(/Welcome to\s+Revanta AI\.?/i, "Revanta AI ki welcome ≡ƒÿä");
  }
  return t.length > 120 ? `${t.slice(0, 115)}ΓÇª ≡ƒÿä` : `Bro, ${t}`;
}
...
function toBusinessReply(text: string) {
  ...
  return t.length > 300 ? t.slice(0, 297) + "ΓÇª" : t;
}
...
function toMixedReply(text: string) {
  ...
  if (/^Book your slot here:/i.test(t)) {
    return `Sure bro ≡ƒÿä ${t}`;
  }
  if (t.length > 180) return `Hey! ${t.slice(0, 175)}ΓÇª`;
  return `Hey bro ≡ƒÿä ${t}`;
}
...
function applyModeStyle(params: {
  ...
  const opener = params.preferredAddress ? `Hey ${params.preferredAddress} ≡ƒÿä` : `Hey ≡ƒÿä`;
...
export async function processIncomingWhatsAppMessage(params: {
  ...
  const { contact: contactRecord, created: contactCreated } = await findOrCreateContact({
    organizationId: params.organizationId,
    leadId: lead.id,
    phone,
    name: params.name || null,
    companyId: (lead as any).companyId || null,
    autoCreateContacts: integration?.autoCreateContacts !== false
  });

  console.log("[WA_CONTACT_CREATE]", {
    organizationId: params.organizationId,
    leadId: lead.id,
  ...
  console.log("[WA_CONVERSATION_CREATE]", {
    organizationId: params.organizationId,
    conversationId: conversation.id,
  ...
  const updatedConversation = await prisma.conversation.update({
    ...
    data: {
      leadId: lead.id,
      companyId: (lead as any).companyId || conversation.companyId || undefined,
      contactId: contactRecord?.id || conversation.contactId || undefined,
      assignedToId: (lead as any).ownerId || integration?.defaultAssigneeId || conversation.assignedToId || undefined,
      lastMessageAt: inboundMessage.createdAt,
      status: "OPEN"
    }
  });

  console.log("[WA_CONVERSATION_UPDATE]", {
    organizationId: params.organizationId,
  ...
  console.log("[TIMER_START] processIncomingWhatsAppMessage", {
    conversationId: updatedConversation.id,
    flowStep: updatedConversation.metadata?.flowStep,
  ...
  return { lead, conversation: updatedConversation };
}
...
export async function resolveOrganizationFromPhoneNumberId(phoneNumberId: string, businessAccountId?: string | null) {
  const lookup = await findWhatsAppIntegrationByIdentifiers({ phoneNumberId, businessAccountId });
  console.log("[WA_ORG_RESOLVE]", {
    phoneNumberId,
    businessAccountId: businessAccountId || null,
    organizationId: lookup.integration?.organizationId || null,
    strategy: lookup.strategy
  });
  return lookup.integration?.organizationId || null;
}
